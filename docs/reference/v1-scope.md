...
| [5](https://github.com/llm4s/llm4s/issues/1132) | `llm4s-ollama`, `llm4s-gemini`, `llm4s-anthropic`, `llm4s-openai` (the first provider modules) | in the build, unpublished |
...
| `llmconnect/provider` — OpenRouter, DeepSeek, Z.ai (OpenAI-compatible, each its own SDK-free client) | one small module each, after `llm4s-openai` | Frozen at 1.0 |
