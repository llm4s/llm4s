...
class DocumentExtractorEdgeCasesSpec extends AnyFlatSpec with Matchers {
  ...
  val extractor: DocumentExtractor = TikaDocumentExtractor

  private def withDocumentExtractorLoggerSilenced[A](body: => A): A = {
    val logger   = LoggerFactory.getLogger(TikaDocumentExtractor.getClass).asInstanceOf[LBLogger]
  ...
  "TikaDocumentExtractor" should "return Left for corrupted PDF content" in {
