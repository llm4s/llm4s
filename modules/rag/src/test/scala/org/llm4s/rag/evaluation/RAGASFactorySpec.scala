...
class RAGASFactorySpec extends AnyFlatSpec with Matchers {
  ...
  it should "resolve dimensions before building any client, so a failure leaks nothing" in {
    // A registry with no chat providers: had the LLM client been built first, the error
    // would be "not registered". Getting the dimensions error proves the lookup ran first,
    // before anything that holds resources existed.
    given ProviderRegistry = ProviderRegistry.ofEmbeddings(FixtureEmbeddings)

    val error = RAGASFactory.basicFromConfigs(chat, embeddings("my-custom-embedder")).left.map(_.formatted)
