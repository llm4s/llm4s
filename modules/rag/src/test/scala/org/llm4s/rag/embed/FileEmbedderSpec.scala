...
class FileEmbedderSpec extends AnyFunSuite with Matchers {
  ...
  private val defaultChunking = TextChunkingConfig(enabled = false, size = 1000, overlap = 100)
  ...
  test("encodeFromPath should encode text file content") {
    withTempFile(".txt", "Hello world") { file =>
      val result = FileEmbedder.encodeFromPath(
        file.toPath,
        mockClient,
        FileEmbeddingConfig(
  ...
  test("encodeFromPath should chunk text when chunking is enabled") {
    ...
    withTempFile(".txt", longText) { file =>
      val chunkingConfig = TextChunkingConfig(enabled = true, size = 500, overlap = 50)
