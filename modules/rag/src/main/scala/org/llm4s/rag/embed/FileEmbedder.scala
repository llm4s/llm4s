...
object TextChunkingConfig {

  /** Embed the whole extracted text as a single unit. */
  val disabled: TextChunkingConfig = TextChunkingConfig(enabled = false, size = 0, overlap = 0)
}

/**
 * Everything [[FileEmbedder]] needs beyond the file itself and the client that embeds it.
 *
 * Grouping these means a caller configures the embedder once rather than repeating five
 * arguments - including a stubs toggle that is a deployment decision, not a call-site one -
 * at every call.
 *
 * @param textModel         Model configuration (name + dimensions) used for text-like files.
 * @param localModels       Model configurations for the image, audio and video modalities.
 * @param chunking          Text chunking settings; if enabled, extracted text is split before embedding.
 * @param experimentalStubs When `false`, image/audio/video files are read and forwarded to
 *                          `client.embedMultimodal()` for real provider embeddings. When `true`,
 *                          deterministic stub vectors are returned instead.
 * @param maxMediaFileSize  Maximum allowed media file size in bytes. Files exceeding this limit
 *                          are rejected with an error. Defaults to 50 MB.
 */
final case class FileEmbeddingConfig(
  textModel: EmbeddingModelConfig,
  localModels: LocalEmbeddingModels,
  chunking: TextChunkingConfig = TextChunkingConfig.disabled,
  experimentalStubs: Boolean = false,
  maxMediaFileSize: Long = FileEmbedder.DEFAULT_MAX_MEDIA_FILE_SIZE
)

/**
 * FileEmbedder handles extracting content from various file types and passing
 ...
 *    extracted by [[org.llm4s.extract.TikaDocumentExtractor]], optionally chunked,
 *    then embedded via the supplied `EmbeddingClient`. Real embeddings are always
 *    produced.
 *
 *  - **Image / Audio / Video**: behaviour depends on `config.experimentalStubs`.
 *    When `false`, the file bytes are read (bounded by `config.maxMediaFileSize`)
...
object FileEmbedder {
  ...
  private val AudioSeedXor = 0x9e3779b97f4a7c15L
  private val VideoSeedXor = 0xc2b2ae3d27d4eb4fL
   ...
   * @param client Embedding client used for text files, and for multimodal files when
   *               `config.experimentalStubs` is `false`.
  ...
  def encodeFromPath(
    ...
    val mime = TikaDocumentExtractor.detectMimeType(f)
    logger.debug(s"[FileEmbedder] MIME detected: $mime")

    if (TikaDocumentExtractor.canExtract(mime)) encodeTextFile(f, mime, client, config)
    else if (mime.startsWith("image/")) encodeMediaFile(f, mime, Image, client, config, seedXor = 0L)
    else if (mime.startsWith("audio/")) encodeMediaFile(f, mime, Audio, client, config, AudioSeedXor)
    else if (mime.startsWith("video/")) encodeMediaFile(f, mime, Video, client, config, VideoSeedXor)
  ...
  private def encodeMediaFile(
    ...
    val modelResult = ModelSelector.selectModel(modality, config.localModels)

    if (config.experimentalStubs) {
      ...
      val maxMediaFileSize = config.maxMediaFileSize
