...
class EmbedxV2Spec extends AnyFunSuite with Matchers {
  ...
  test("Non-text: image/audio/video → 501 by default (stubs disabled)") {
    ...
    val imgRes: Result[Seq[EmbeddingVector]] = withTempFile("embedx_png_", ".png") { png =>
      writeDummyPng(png)
      UniversalEncoder.encodeFromPath(png, stubClient, dummyModel, dummyChunking, experimentalStubsEnabled = false)
    }

    val audRes = withTempFile("embedx_wav_", ".wav") { wav =>
      writeDummyWav(wav)
      UniversalEncoder.encodeFromPath(wav, stubClient, dummyModel, dummyChunking, experimentalStubsEnabled = false)
    }

    val vidRes = withTempFile("embedx_mp4_", ".mp4") { mp4 =>
      writeDummyMp4(mp4)
      UniversalEncoder.encodeFromPath(mp4, stubClient, dummyModel, dummyChunking, experimentalStubsEnabled = false)
    }
  ...
  test("Experimental stubs: image/audio/video produce vectors and experimental=true") {
    ...
    val imgRes = withTempFile("embedx_png_", ".png") { png =>
      writeDummyPng(png)
      UniversalEncoder.encodeFromPath(png, stubClient, dummyModel, dummyChunking, experimentalStubsEnabled = true)
    }

    val audRes = withTempFile("embedx_wav_", ".wav") { wav =>
      writeDummyWav(wav)
      UniversalEncoder.encodeFromPath(wav, stubClient, dummyModel, dummyChunking, experimentalStubsEnabled = true)
    }

    val vidRes = withTempFile("embedx_mp4_", ".mp4") { mp4 =>
      writeDummyMp4(mp4)
      UniversalEncoder.encodeFromPath(mp4, stubClient, dummyModel, dummyChunking, experimentalStubsEnabled = true)
    }
  ...
  test("Local model dimensions (Image/Audio/Video) match registry") {
    val imgModel = ModelSelector.selectModel(Image).fold(err => fail(err.formatted), identity)
    val audModel = ModelSelector.selectModel(Audio).fold(err => fail(err.formatted), identity)
    val vidModel = ModelSelector.selectModel(Video).fold(err => fail(err.formatted), identity)
