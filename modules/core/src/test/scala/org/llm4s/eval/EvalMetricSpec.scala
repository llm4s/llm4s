...
class EvalMetricSpec extends AnyFlatSpec with Matchers {
  ...
  "Faithfulness.strict" should "use higher threshold" in {
    Faithfulness.strict.toOption.get
      .evaluate(EvalContext("Q", Seq("C"), "A"), new MockLLMClient("SCORE: 0.85"))
      .toOption
      .get
  ...
  "Faithfulness safe constructor" should "return error for invalid threshold" in {
    Faithfulness(-0.1).isLeft shouldBe true
    Faithfulness(1.5).isLeft shouldBe true
    Faithfulness(0.5).isRight shouldBe true
  }
  ...
  "AnswerRelevance safe constructor" should "return error for invalid threshold" in {
    AnswerRelevance(-0.1).isLeft shouldBe true
    AnswerRelevance(1.5).isLeft shouldBe true
  }
  ...
  "ContextPrecision safe constructor" should "return error for invalid threshold" in {
    ContextPrecision(-0.1).isLeft shouldBe true
    ContextPrecision(1.5).isLeft shouldBe true
  }
