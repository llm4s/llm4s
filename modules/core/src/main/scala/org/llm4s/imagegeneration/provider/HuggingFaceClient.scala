...
class HuggingFaceClient(config: HuggingFaceConfig, httpClient: HttpClient) extends ImageGenerationClient {
  ...
  override def generateImageAsync(
    ...
    Future(blocking(generateImage(prompt, options)))(HuggingFaceClient.blockingEc)
  ...
  override def generateImagesAsync(
    ...
    Future(blocking(generateImages(prompt, count, options)))(HuggingFaceClient.blockingEc)
  ...
  override def editImageAsync(
    ...
    Future(blocking(editImage(imagePath, prompt, maskPath, options)))(HuggingFaceClient.blockingEc)
