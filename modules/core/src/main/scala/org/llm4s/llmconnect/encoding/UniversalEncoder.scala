...
object UniversalEncoder {
  ...
  /** Configuration for text embeddings chunking. */
  ...
  def encodeFromPath(
    ...
    else if (mime.startsWith("image/")) encodeImageFile(f, mime, experimentalStubsEnabled)
    else if (mime.startsWith("audio/")) encodeAudioFile(f, mime, experimentalStubsEnabled)
    else if (mime.startsWith("video/")) encodeVideoFile(f, mime, experimentalStubsEnabled)
  ...
  private def encodeImageFile(
    ...
    val modelResult = ModelSelector.selectModel(Image)
  ...
  private def encodeAudioFile(
    ...
    val modelResult = ModelSelector.selectModel(Audio)
    modelResult.map { model =>
      val dim = model.dimensions
      // Limit dimension to prevent OOM in tests (max 8K dimensions)
      val safeDim = math.min(dim, MAX_STUB_DIMENSION)
  ...
  private def encodeVideoFile(
    ...
    val modelResult = ModelSelector.selectModel(Video)
    modelResult.map { model =>
      val dim = model.dimensions
      // Limit dimension to prevent OOM in tests (max 8K dimensions)
      val safeDim = math.min(dim, MAX_STUB_DIMENSION)
