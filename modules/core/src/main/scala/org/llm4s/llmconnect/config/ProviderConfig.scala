...
object OpenAIConfig {
  ...
  /** Construct OpenAIConfig from already-resolved values (used by Llm4sConfig). */
  def fromValues(
    modelName: String,
    apiKey: String,
    organization: Option[String],
  ...
  private def getContextWindowForModel(modelName: String): (Int, Int) = {
    ...
    val registryResult =
      ModelRegistry.lookup("openai", modelName).toOption
...
object AzureConfig {
  ...
  /** Construct AzureConfig from already-resolved values (used by Llm4sConfig). */
  def fromValues(
    modelName: String,
    endpoint: String,
    apiKey: String,
...
object AnthropicConfig {
  ...
  private def getContextWindowForModel(modelName: String): (Int, Int) = {
    ...
    val registryResult =
      ModelRegistry.lookup("anthropic", modelName).toOption
        .orElse(ModelRegistry.lookup(modelName).toOption)
  ...
  /** Construct AnthropicConfig from already-resolved values (used by Llm4sConfig). */
  def fromValues(
    modelName: String,
    apiKey: String,
    baseUrl: String
...
object OllamaConfig {
  ...
  private def getContextWindowForModel(modelName: String): (Int, Int) = {
    ...
    val registryResult =
      ModelRegistry.lookup("ollama", modelName).toOption
        .orElse(ModelRegistry.lookup(modelName).toOption)
  ...
  /** Construct OllamaConfig from already-resolved values (used by Llm4sConfig). */
  def fromValues(
    modelName: String,
    baseUrl: String
  ): OllamaConfig = {
