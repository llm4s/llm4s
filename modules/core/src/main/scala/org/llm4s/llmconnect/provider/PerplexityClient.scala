...
/**
 * Perplexity AI LLM client implementation using the OpenAI-compatible Sonar API.
 *
 * Perplexity Sonar models combine LLM reasoning with real-time web search,
 * returning grounded answers with citations. This makes them valuable for
 * enterprise use cases where up-to-date information is required (market research,
 * news monitoring, competitive intelligence).
 *
 * Supported models:
 *  - `sonar` — cheapest, good for general queries
 *  - `sonar-pro` — higher quality, larger context
 *  - `sonar-reasoning` — reasoning-enhanced Sonar
 *  - `sonar-reasoning-pro` — highest quality reasoning with web search
 *
 * Note on citations: Perplexity responses may include a `citations` field not
 * present in standard OpenAI format. In v1 this field is ignored; the response
 * text is returned as-is.
 *
 * @param config         Perplexity configuration containing API key, model, base URL, and context settings.
 * @param metrics        MetricsCollector for recording request metrics.
 * @param exchangeLogging Provider exchange logging configuration.
 */
class PerplexityClient(
  ...
  protected def clientDescription: String = s"Perplexity client for model ${config.model}"
  protected def providerName: String      = "perplexity"
  ...
  override def complete(
    ...
    logger.debug(s"Sending request to Perplexity API at ${config.baseUrl}/chat/completions")
...
object PerplexityClient {
  ...
  )(using ModelRegistryService): Result[PerplexityClient] =
    Try(new PerplexityClient(config, metrics, exchangeLogging)).toResult

  def apply(config: PerplexityConfig, metrics: MetricsCollector)(using ModelRegistryService): Result[PerplexityClient] =
    Try(new PerplexityClient(config, metrics)).toResult

  def apply(config: PerplexityConfig)(using ModelRegistryService): Result[PerplexityClient] =
    Try(new PerplexityClient(config, MetricsCollector.noop)).toResult
}
