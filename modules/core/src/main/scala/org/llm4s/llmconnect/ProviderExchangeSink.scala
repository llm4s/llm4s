...
object ProviderExchangeSink {
  ...
  private def prepareDirectory(directory: Path): Result[Path] =
    if (Files.exists(directory)) {
      if (Files.isDirectory(directory)) Right(directory)
...
private object ProviderExchangeJson {
  def toJson(exchange: ProviderExchange): ujson.Obj =
    ujson.Obj(
      ...
      "model"          -> ujson.Str(exchange.model.getOrElse("")),
      "request_id"     -> ujson.Str(exchange.requestId.getOrElse("")),
      "correlation_id" -> ujson.Str(exchange.correlationId.getOrElse("")),
