...
/**
 * Internal PureConfig-based loader for provider configuration.
 *
 * This is kept separate from Llm4sConfig to keep that façade slim; external
 * code should use Llm4sConfig.provider(), not this object directly.
 */
private[config] object ProviderConfigLoader {

  // ---- Internal shapes that mirror llm4s.* provider config ----

  private final case class LlmSection(model: Option[String])

  private final case class OpenAISection(
    baseUrl: Option[String],
    apiKey: Option[String],
    organization: Option[String]
  )

  private final case class AzureSection(
    endpoint: Option[String],
    apiKey: Option[String],
    apiVersion: Option[String]
  )

  private final case class AnthropicSection(
    baseUrl: Option[String],
    apiKey: Option[String]
  )

  private final case class OllamaSection(
    baseUrl: Option[String]
  )

  private final case class ProviderRoot(
    llm: LlmSection,
  ...
  private implicit val llmSectionReader: PureConfigReader[LlmSection] =
    PureConfigReader.forProduct1("model")(LlmSection.apply)

  private implicit val openAISectionReader: PureConfigReader[OpenAISection] =
    PureConfigReader.forProduct3("baseUrl", "apiKey", "organization")(OpenAISection.apply)

  private implicit val azureSectionReader: PureConfigReader[AzureSection] =
    PureConfigReader.forProduct3("endpoint", "apiKey", "apiVersion")(AzureSection.apply)

  private implicit val anthropicSectionReader: PureConfigReader[AnthropicSection] =
    PureConfigReader.forProduct2("baseUrl", "apiKey")(AnthropicSection.apply)

  private implicit val ollamaSectionReader: PureConfigReader[OllamaSection] =
    PureConfigReader.forProduct1("baseUrl")(OllamaSection.apply)

  private implicit val providerRootReader: PureConfigReader[ProviderRoot] =
    PureConfigReader.forProduct5("llm", "openai", "azure", "anthropic", "ollama")(ProviderRoot.apply)

  // ---- Public API used by Llm4sConfig ----

  /** Load ProviderConfig from the given ConfigSource under llm4s.*. */
  ...
  /** Resolve the shared OpenAI API key (used by both chat and embeddings). */
  ...
  private def buildOllamaConfig(modelName: String, section: Option[OllamaSection]): Result[ProviderConfig] =
    section match {
      case Some(ollama) =>
        ...
        baseUrlResult.map { baseUrl =>
          OllamaConfig.fromValues(modelName, baseUrl)
        }
