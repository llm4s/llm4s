 ...
 * `/chat/completions` API, and DeepSeek (`deepseek`), each running on
...
final class Llm4sOpenAICompatibleModule extends Llm4sProviderModule:
  override def chatProviders: Seq[ProviderDescriptor] = Seq(OpenAICompatibleProvider, DeepSeekProvider)
