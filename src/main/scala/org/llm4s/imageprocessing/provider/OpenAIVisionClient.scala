...
import org.llm4s.core.LLMError
...
class OpenAIVisionClient(config: OpenAIVisionConfig) extends org.llm4s.imageprocessing.ImageProcessingClient {
  ...
  override def analyzeImage(
    ...
    try {
      ...
      val base64Image = encodeImageToBase64(imagePath) match {
        ...
        case Failure(exception) => return Left(LLMError.ProcessingError(s"Failed to encode image: ${exception.getMessage}"))
      }
      ...
      val visionResponse = callOpenAIVisionAPI(base64Image, analysisPrompt) match {
        ...
        case Failure(exception) => return Left(LLMError.APIError(s"OpenAI Vision API call failed: ${exception.getMessage}"))
      }
  ...
  private def extractTagsFromText(text: String): List[String] = {
    ...
    val commonTags = Set(
      ...
      "outdoor", "indoor", "landscape", "portrait", "nature",
      "city", "street", "water", "sky", "mountain", "beach",
      "food", "restaurant", "kitchen", "bedroom", "living room",
      "technology", "computer", "phone", "book", "art", "painting"
    )
  ...
  private def extractObjectsFromText(text: String): List[DetectedObject] = {
    ...
    objectKeywords.filter(keyword => text.toLowerCase.contains(keyword))
  ...
  private def extractTextFromResponse(response: String): Option[String] = {
    ...
    textPatterns.flatMap(_.findFirstMatchIn(response.toLowerCase))
      .headOption
      .map(_.group(1))
  }
}
