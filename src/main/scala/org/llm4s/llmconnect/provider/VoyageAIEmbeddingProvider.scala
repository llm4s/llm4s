...
import org.llm4s.error.{LLMError => LErr} // <-- Adjust import/constructors if your repo differs.
...
object VoyageAIEmbeddingProvider extends EmbeddingProvider {
  ...
  /** Parse VoyageAI embeddings JSON into EmbeddingResponse
   * Typical Voyage response:
   * { "data": [{ "embedding": [..], "index": 0 }], "model": "voyage-3-large-2", ... }
   */
  private def parseSuccess(body: String, model: String): Result[EmbeddingResponse] = {
    try {
      ...
      val vectors: Vector[Array[Float]] =
        data.toVector.map { item =>
          val emb = item("embedding").arr
          val arr = new Array[Float](emb.length)
          var i   = 0
          while (i < emb.length) { arr(i) = emb(i).num.toFloat; i += 1 }
          arr
        }
      ...
      case e: Throwable =>
        Left(LErr.ParseError(message = s"Failed to parse VoyageAI embeddings: ${e.getMessage}"))
    }
  }
}
